...
function getAverageColor(imgdata) { /* Calculate average RGB color of the image */
var r = 0;
var b = 0;
var g = 0;
...
let ans = Math.sqrt((arrayAns[0]-arrayGuess[0])**2+(arrayAns[1]-arrayGuess[1])**2+(arrayAns[2]-arrayGuess[2])**2);
return ans;
}
...
authorname.innerHTML = (jsondata.user.first_name + " " + jsondata.user.last_name);
...
var averageColor = getAverageColor(context.getImageData(0, 0, canvas.width, canvas.height).data);
//finds average color
...
document.getElementById("guessedColor").style.background = `rgb(${guessedColor})`
document.getElementById("actualColor").style.background=`rgb(${averageColor})`

document.getElementById("playerGuessText").innerHTML = `You guessed: (${guessedColor})`
document.getElementById("actualGuessText").innerHTML = `Actual color: (${averageColor})`
